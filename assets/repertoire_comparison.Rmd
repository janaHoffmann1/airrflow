...
ggplot_theme <- theme_bw(base_family = "ArialMT") +
            theme(panel.grid.major = element_blank(),
            panel.border = element_rect(colour = "black", fill=NA, size=0.5),
            panel.grid.minor = element_blank(), text = element_text(family="ArialMT"))
...
subdirectory [clonal_analysis/repertoire_analysis/repertoire_analysis_report/repertoires](clonal_analysis/repertoire_analysis/repertoire_analysis_report/repertoires).
...
- Report summarizing the repertoire properties for all the samples [clonal_analysis/repertoire_analysis/repertoire_anaysis_report/index.html](clonal_analysis/repertoire_analysis/repertoire_anaysis_report/index.html).
...
# Seqeuencing depth
Reads per UMI plotted for each of the samples.
```{r reads_per_UMI_plot, echo=FALSE, warning=FALSE, results='asis'}
tryCatch( {
    tab_seq  <- read.table("reads_per_UMI.csv", header=TRUE, sep=",")
    tab_seq[is.na(tab_seq)] <- 0
    plot_table <- tidyr::pivot_longer(tab_seq,
                                      cols=-X,
                                      names_to = "sample_id",
                                      values_to ="count")

    x_range <- c(floor(min(plot_table$X)), ceiling(max(plot_table$X)))
    y_max <- ceiling(max(log10(plot_table$count)))
    UMI_plot <- ggplot(data=plot_table,
                        aes(x=X, y=count, group=sample_id)) +
                  geom_line(aes(colour=sample_id)) +
                  geom_point(aes(colour=sample_id)) +
                  xlab('Reads per UMI') + ylab('UMI count') +
                  scale_x_reverse(breaks=seq(x_range[1], x_range[2], by=ceiling(x_range[2]/10))) +
                  scale_y_continuous(
                    trans=scales::pseudo_log_trans(sigma=1, base=10),
                    breaks=c(0,10^seq(1, y_max,length.out=max(3,y_max)))
                  )
  ...
  error=function(e){message("No reads per UMI information available.")}
)
```
...
tryCatch( {
    ...
    plot_table <- tidyr::pivot_longer(tab_seqs,
                                      cols=Sequences:Representative_2,
                                      names_to = "steps",
                                      values_to ="count")
...
if (is.null(tab_seqs_assembled$sample_id)) {
  tab_seqs_assembled$sample_id <- sapply(tab_seqs_assembled$input, function(x) {
    if (grepl("_", x)) {
      unlist(strsplit(as.character(x), "_(?=[^_]+$)", perl=TRUE))[1]
    } else if (grepl("\\.", x)) {
      unlist(strsplit(as.character(x), "\\.(?=[^\\.]+$)", perl=TRUE))[1]
    } else {
      x
    }
  })
...
The V gene usage in each of the samples is represented below. All plots and tables can be found [here](./repertoire_comparison/V_family/).

```{r gene_family, echo=F}
family <- countGenes(df_all, gene="v_call", groups=c("sample_id","subject_id","locus"), mode="family")
...
The V gene usage in each of the samples is represented below.
All plots and tables can be found [here](./repertoire_comparison/V_family/).

```{r gene_seq, echo=F, fig.width=10, fig.height=5}
family <- countGenes(df_all, gene="v_call", groups=c("sample_id","subject_id","locus"), mode="gene")
...
If you use nf-core/airrflow for your analysis, please cite our paper:

> Gisela Gabernet, Susanna Marquez, Robert Bjornson, Alexander Peltzer, Hailong Meng, Edel Aron, Noah Y. Lee, Cole G. Jensen, David Ladd, Mark Polster, Friederike Hanssen, Simon Heumos, nf-core community, Gur Yaari, Markus C. Kowarik, Sven Nahnsen, Steven H. Kleinstein. nf-core/airrflow: an adaptive immune receptor repertoire analysis workflow employing the Immcantation framework. (2024) PLOS Computational Biology, 20(7), e1012265. doi: [https://doi.org/10.1371/journal.pcbi.1012265](https://doi.org/10.1371/journal.pcbi.1012265). Pubmed PMID: 39058741.
