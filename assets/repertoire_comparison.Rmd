...
tryCatch( {
    ...
    y_trans <- scales::pseudo_log_trans(sigma=1, base=10)
    y_breaks <- scales::trans_breaks(y_trans$transform, y_trans$inverse)(range(plot_table$count, na.rm=TRUE))
    y_breaks <- y_breaks[y_breaks >= min(plot_table$count, na.rm=TRUE) & y_breaks <= max(plot_table$count, na.rm=TRUE)]
    p <- ggplotly(seqs_plot) %>%
      layout(yaxis = list(
        tickmode = "array",
        tickvals = y_trans$transform(y_breaks),
        ticktext = scales::label_number(big.mark = "")(y_breaks)
      ))
  p
...
y_trans <- scales::pseudo_log_trans(sigma=1, base=10)
y_breaks <- scales::trans_breaks(y_trans$transform, y_trans$inverse)(range(tab_seqs_assembled$output_size, na.rm=TRUE))
y_breaks <- y_breaks[y_breaks >= min(tab_seqs_assembled$output_size, na.rm=TRUE) & y_breaks <= max(tab_seqs_assembled$output_size, na.rm=TRUE)]
